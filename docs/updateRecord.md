...
> **本文件已归档，不再更新。** 更新日志已迁移至 `docs/changelog/` 目录（每版本一个 Markdown 文件），后台管理「更新日志」页与 `GET /api/admin/changelog` 接口均以该目录为数据源。设计见 `docs/changelog-design.md`。

### 20260829

更新至V2.0.0

1. 管理界面「Instrument Panel」全站重构：暗色/亮色双主题、全新 UI 组件库与移动端适配，原生弹窗全部替换为统一的 Toast/Dialog 反馈体系
2. 接口文档专业化：集成 Swagger UI，全部路由补齐 OpenAPI schema，文档由代码自动生成，与实际接口永不脱节
3. 新增插件市场：从远程源一键安装/更新/卸载插件，支持 sha256 完整性校验、GitHub 加速兜底与安装进度实时展示
4. 新增验证码识别代理：源直接请求主服务相同端口即可调用 captcha-bypass（ddddocr）插件的 OCR/滑块/旋转识别，无需感知插件端口
5. 文件管理预览接入只读 monaco 编辑器，支持 40+ 种语言语法高亮，暗色/亮色主题跟随
6. 修复十余处内存与资源泄漏（定时器堆积、连接池失效、子进程孤儿、缓存无界增长等），长时间运行更稳定
7. 大规模重构收敛：路径校验/流媒体重写/AI 对话等重复实现统一收敛，清理死代码与失效依赖，新增零依赖测试体系（68 用例）
8. 修复 lx 音乐代理带参数请求被误杀中断、站源映射页面误报「未保存」等问题；访问 .env 等受保护文件的提示更加明确
9. 站源映射新增「撤销修改」按钮，防止误保存
...
1. 新增了一些通过ai mcp工具写的源
2. drplayer前端重构升级，适配了移动端

### 20260418

更新至V1.4.4

1. 专为AI写源而生，提供了完美的技能支持，MCP工具条调优
2. 处理了MCP和bundle，修复了关键问题点

### 20260322

更新至V1.4.3

1. 重写了不夜多线程代理，支持了皮卡丘上的MPV播放器，兼容影图的IJK和EXO播放器。
2. 新版jar多线程代理已通过迅雷测试，迅雷有个坑，没登录的只能试看前几分钟，进度播放到后面或者手动拖到后面都会GG，登录后猫原画没问题

### 20260321

更新至V1.4.2

1. 尝试兼容IOS，支持18-26版本nodejs (如果没兼容好别骂，尽力了)
2. 尝试兼容影视+、皮卡丘旧版的错误启动方式(做容错了，保留旧版壳子用户权益，不能剥夺旧版壳子升级最新DS的需求)
3. 修复了RSA在node24以上版本不兼容PCSK1解密协议问题导致APP影视模板.js分发的源不可用问题(重要)
4. 后台管理继续打磨，4层保底使用SQLite,最明确方式获取当前项目目录，修复上个版本在大部分市面壳子上出现服务异常的问题
5. 加了点隐藏功能，一般人应该发现不了
6. 增加了RCE攻击防护
7. 合并了迅雷原画推送播放逻辑
8. 修复了IOS用户后台管理编辑环境变量无法保存问题
9. 优化了docker自动打包兼容性
10. 后台管理新增了加解密工具
11. 支持手机运行的py二进制环境识别
